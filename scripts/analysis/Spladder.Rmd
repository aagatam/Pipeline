...
packages<-c("knitr","RColorBrewer","VennDiagram","veccompare","ggplot2","pals","dplyr","matrixStats","gplots","UpSetR","XML","yaml", "tmod","tidyverse","gridExtra")
...
BiocManager::install()}
listOfBiocPackages<-c("topGO","GOfuncR","annotate","biomaRt","GOfuncR",config$GO_MAPPING)
...
dir.create(file.path(spl_path, 'Script_output'), showWarnings = FALSE)
dir.create(file.path(bb_path, 'Filtered'), showWarnings = FALSE)
...
# Section 1 
## Number of all and common alternative splicing events, when `r th` samples have to fullfil conditions:
- New/Old PSI >`r PSI_NaO` and sd < `r sd_th`
- New/new and old/old sd < `r sd_th`
...
colnames(tab_all)<-c('both iso known','both iso known_common','%','both iso new','both iso new common','%','new and old','new and old common','%','Total')

rownames(tab_all)<-c('Alt_3_prime','Alt_5_prime','exon_skip','mutex_exons','mult_exon_skip')
kable(tab_all)
```

## Number of all and common genes containing alternative splicing events:
...
colnames(tab_allg)<-c('both iso known','both iso known_common','%','both iso new','both iso new common','%','new and old','new and old common','%','Total')
rownames(tab_allg)<-c('Alt_3_prime','Alt_5_prime','exon_skip','mutex_exons','mult_exon_skip')
kable(tab_allg)
```

# Section 2 
## Upset plots for genes containing ASEs
...
grid.text("Overlap for genes containing common ASE for new+old",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
grid.text("Overlap for genes containing common ASE for old+old",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
grid.text("Overlap for genes containing common ASE for new+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(list_BN, paste(spl_path,'/Script_output/BN_common_genes.txt',sep=''))
...
writeLines(list_BK, paste(spl_path,'/Script_output/BK_common_genes.txt',sep=''))
...
writeLines(list_NAO, paste(spl_path,'/Script_output/NaO_common_genes.txt',sep=''))

```

# Section 3
## Venn diagrams showing genes overlap between groups
...
#prepare for InterProScan
events_interpro<-c('alt_3prime','alt_5prime','exon_skip','mult_exon_skip','mutex_exons')
i=1
for (event in events)
{

  bisbee<-get(paste(event,'_bisbee',sep=''))
  bisbee_prot<-get(paste(event,'_bisbee_pep',sep=''))
  for (group in c('_both_new','_both_known','_newAndOld'))
  {
    spladder<-get(paste(event,group,'_common',sep=''))
    Bisbee_filtered<-bisbee[bisbee$event_id%in%spladder$event_id,]
    name_filtered<-paste(event,group,'_bisbee_filtered',sep='')
    assign(name_filtered,Bisbee_filtered)
    Bisbee_filtered_prot<-bisbee_prot[bisbee_prot$event_id%in%spladder$event_id,]
    name_filtered_prot<-paste(event,group,'_bisbee_prot_filtered',sep='')
    assign(name_filtered_prot,Bisbee_filtered_prot)
    setwd(paste(bb_path,'/Filtered',sep=''))
    write.csv(Bisbee_filtered,name_filtered)
    write.csv(Bisbee_filtered_prot,name_filtered_prot)
    Bisbee_filtered$event_id<-sub('(\\.)(\\d+$)','\\_\\2',Bisbee_filtered$event_id)
    pasta<-paste('>.+',paste(Bisbee_filtered$event_id,collapse='\\D.+\\n[A-Z,\\n]+|>.+'),'\\D.+\\n[A-Z,\\n]+',sep='')
    pasta<-paste(paste(Bisbee_filtered$event_id,collapse='\\D\n'),'\\D',sep='')
    cat(pasta,file=paste(bb_path,'/',events_interpro[i],group,'_bisbee_filtered','_to_grep.txt',sep=''),sep='')
  }
  i=i+1
}

#prepare tab for plots
gene_names<-(unique(c(mult_exon_skip_newAndOld_bisbee_prot_filtered$gene,mutex_exons_newAndOld_bisbee_prot_filtered$gene,alt_5_prime_newAndOld_bisbee_prot_filtered$gene,
                    alt_3_prime_newAndOld_bisbee_prot_filtered$gene,exon_skip_newAndOld_bisbee_prot_filtered$gene)))


group<-'newAndOld'
tab<-data.frame(row.names = gene_names)
tab$events<-''
tab$effect_orf<-''
tab$effect_aa<-''
i=1
for (gene in gene_names)
{
  for (event in events )
  {
    name_filtered_prot<-paste(event,group,'bisbee_prot_filtered',sep='_')
    data<-get(name_filtered_prot)
    data<-data[which(data$topEffect=='True'),]
    idx<-which(data$gene==gene)
    if (length(idx!=0))
    {
    tab$events[i]<-paste(tab$events[i], paste(data$event_id[idx],collapse=' '), sep=' ')
    tab$effect_orf[i]<-paste(tab$effect_orf[i], paste(data$orf_effect[idx],collapse=' '), sep=' ')
    tab$effect_aa[i]<-paste(tab$effect_aa[i], paste(data$aa_effect[idx],collapse=' '), sep=' ')
    }
  }
  i=i+1
}
...
ensemble2gene <- getBM(attributes=c("ensembl_gene_id","external_gene_name"),
                       ...
                       mart = mart)
group<-'both_new'
tab_BN<-data.frame(row.names = ensemble2gene$ensembl_gene_id)
tab_BN$events<-''
i=1
for (gene in ensemble2gene$ensembl_gene_id)
{
  for (event in events )
  {
    name_filtered_prot<-paste(event,group,'common',sep='_')
    data<-get(name_filtered_prot)
    idx<-which(data$gene_name==gene)
    if (length(idx!=0))
    {
      tab_BN$events[i]<-paste(tab_BN$events[i], paste(data$event_id[idx],collapse=' '), sep=' ')
    }
  }
  i=i+1
}
tab$gene_name<-rownames(tab)
tab_BN$gene_name<-ensemble2gene$external_gene_name
tab_merged_NN_ON<-merge(tab, tab_BN,by.x='gene_name',by.y='gene_name')
colnames(tab_merged_NN_ON)<-c('gene_name','ON_events','effect_orf','effect_aa','NN_events')
save(tab_merged_NN_ON,alt_3_prime_newAndOld_bisbee_filtered, alt_3_prime_both_new_bisbee_filtered,alt_5_prime_newAndOld_bisbee_filtered,alt_5_prime_both_new_bisbee_filtered,mutex_exons_both_new_bisbee_filtered,mutex_exons_newAndOld_bisbee_filtered,mult_exon_skip_both_new_bisbee_filtered,mult_exon_skip_newAndOld_bisbee_filtered,exon_skip_both_new_bisbee_filtered,exon_skip_newAndOld_bisbee_filtered,file=paste(spl_path,'To_plots.RData',sep='/'))
write.csv(tab_merged_NN_ON,file=paste(spl_path,'To_plots.csv',sep='/'))
```

# Section 4 
## Upset plots for GO terms
  ...
  for (type in c('both_new','both_known','newAndOld'))
  {
    ...
    for (method in c('CC','MF','BP'))
    {
      GOdata <- new("topGOdata", ontology = method, allGenes = geneList2,
                annot = annFUN.org,mapping=config$GO_MAPPING,
                ID='ensembl')
      ...
      lgenes <- get_anno_genes(lnames, database = config$GO_MAPPING,genes=SYM_MM)

      allRes<-allRes[which(as.numeric(allRes$parentchildFisher)<th_GO),]
      p.val.FP <- score(resultFisher)
      lnames <- names(p.val.FP)[p.val.FP < 0.01]
      lgenes <- get_anno_genes(lnames, database =config$GO_MAPPING,genes=data$gene_name_proper)
      ...
      write.csv(allRes[,c(1,2,6,7)],paste(spl_path,'/Script_output/',event,'_',type,'_',th_GO,'_',method,'.csv',sep=''))
...
grid.text("BP terms overlap for old+old",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(BP_BK_i, paste(spl_path,'/Script_output/BP_BK_common.txt',sep=''))
...
grid.text("CC terms overlap for old+old",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(CC_BK_i, paste(spl_path,'/Script_output/CC_BK_common.txt',sep=''))
...
grid.text("MF terms overlap for old+old",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(MF_BK_i, paste(spl_path,'/Script_output/MF_BK_common.txt',sep=''))
...
grid.text("BP terms overlap for new+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(BP_BN_i, paste(spl_path,'/Script_output/BP_BN_common.txt',sep=''))
...
grid.text("CC terms overlap for new+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(CC_BN_i, paste(spl_path,'/Script_output/CC_BN_common.txt',sep=''))
...
grid.text("MF terms overlap for new+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(MF_BN_i, paste(spl_path,'/Script_output/MF_BN_common.txt',sep=''))
...
grid.text("BP terms overlap for old+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(BP_ON_i, paste(spl_path,'/Script_output/BP_ON_common.txt',sep=''))
...
grid.text("CC terms overlap for old+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
...
writeLines(CC_ON_i, paste(spl_path,'/Script_output/CC_ON_common.txt',sep=''))
...
MF_ON<-list('alt_3_prime'=alt_3_prime_newAndOld_common_MF$GO.ID,'alt_5_prime'=alt_5_prime_newAndOld_common_MF$GO.ID,
 ...
 grid.text("MF terms overlap for old+new",x = 0.7, y=0.95, gp=gpar(fontsize=8))
MF_ON_i<-intersect(alt_3_prime_newAndOld_common_MF$Term,intersect(alt_5_prime_both_new_common_MF$Term,intersect(exon_skip_both_new_common_MF$Term,intersect(mutex_exons_both_new_common_MF$Term,mult_exon_skip_both_new_common_MF$Term))))
writeLines(MF_ON_i, paste(spl_path,'/Script_output/MF_ON_common.txt',sep=''))
...
```{r fig.align='left', message=FALSE, include=FALSE,warning=FALSE}
for (event in events)
{
  i=1
  ...
  ggsave(plot = g_new, width = 20, height = 20, dpi = 300,
         filename=paste(spl_path,'/Script_output/GO_',
                        event,'.pdf',sep=''))
